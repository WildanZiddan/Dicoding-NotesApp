...
export const UpsertNote = ({ setOpen, note, createNote, updateNote }) => {
    ...
    const handleSubmit = (event) => {
        ...
        if(note){
            ...
            createNote({ id: getID(), title, desc, createdAt: new Date().toDateString(), });
        }
        clearInputs();
        setOpen(false);
    };
        ...
        <div className="upsert-note">
            <div className="upsert-wrapper">
                ...
                <form className="upsert-form" onSubmit={handleSubmit}>
                    <input required type="text" placeholder="Title" className="input-form" value={title} onChange={(e) => setTitle(e.target.value)} />
                    <textarea placeholder="Enter your note" className="textarea-form" value={desc} onChange={(e) => setDesc(e.target.value)} ></textarea>
