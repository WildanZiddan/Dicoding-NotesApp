...
export const Navbar = ({ setOpen, state, dispatch, palettes, currentPalette, setCurrentPalette }) => {
                ...
                <div className="nav-icon">
                    <div className={`palettes ${onPalette && "active"}`}>
                        {palettes.map((palette) => (
                            <div onClick={() => handlePalette(palette)} key={palette?.id} style={{ backgroundColor: `${palette?.color}` }} className={`palette-item ${currentPalette?.id === palette?.id && "active"}`}></div>
                        ))}    
                    </div>
